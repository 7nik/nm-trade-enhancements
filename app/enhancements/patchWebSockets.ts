...
addPatches((angular) => {
    angular.module("Art").factory("artWebSocket", [
        ...
        (
             ...
             */
            function wrapApply<P extends any[]> (
            ...
            class SortedList implements ISortedList {
                ...
                constructor (namespace: string, options: SortedListOptions) {
                    ...
                    this.#liveList.on("init", (results, count) => {
                        if (options.onLoadInitial) {
                            wrapApply(options.onLoadInitial, this)({ results, count });
                        }
                    ...
                    if (options.onLoad) {
                        this.#liveList.on("load", wrapApply(options.onLoad, this));
                    }
                    if (options.onAddItem) {
                        this.#liveList.on("add", wrapApply(options.onAddItem, this));
                    }
                    if (options.onRemoveItem) {
                        this.#liveList.on("remove", wrapApply(options.onRemoveItem, this));
                    }
